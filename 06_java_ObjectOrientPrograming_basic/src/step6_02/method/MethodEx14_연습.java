...
class NumberGame{
	...
	void moveLeft() {
		...
		if (playerLocation > 0) { 
			move[playerLocation] = 0; // 그 자리는 0이 된다. 
			move[playerLocation - 1] = 8; // 그 자리의 왼쪽이 8이 된다. 
			playerLocation--; // 변수의 인덱스를 하나씩 줄인다.
		}
		else {
	...
	void moveRight() {
		...
		if (playerLocation < move.length - 1) { // 인덱스 길이에 맞춰 설정 
			move[playerLocation] = 0; // 선택한 자리는 0이 된다.
			move[playerLocation + 1] = 8; // 선택한 자리의 오른쪽이 9이 된다.
			playerLocation++; // 변수의 인덱스를 하나씩 늘린다. 
		}
		else {
			System.out.println("더이상 움직일 수 없습니다!");
		}
